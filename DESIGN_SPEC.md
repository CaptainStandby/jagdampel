...
> pointing users to the official Landesjagdverordnung. Shipped season data is transcribed from the
> official regulations (each state's `source` block cites the regulation and `asOf` date), but it is
> **never authoritative** — always verify against the current Landesjagdverordnung.

## 4. Tech stack & rationale

| Layer               | Choice                                                              | Why                                                                                                                                                                                                                                                                                                                                                           |
| ------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Framework           | **Astro 7** (`output: 'static'`)                                    | Ships zero JS by default; hydrate only interactive islands. Ideal for content-heavy + a few widgets on slow connections.                                                                                                                                                                                                                                      |
| Interactive islands | **React 19** via `@astrojs/react` 6                                 | Map and filters as isolated islands.                                                                                                                                                                                                                                                                                                                          |
| Styling             | **Tailwind v4** via `@tailwindcss/vite` (`astro.config.mjs`)        | Utility-first; theme tokens (incl. `jagd-*` colors) defined in `src/styles/global.css` `@theme`. **Note:** v4 dropped the `@astrojs/tailwind` integration — do not reintroduce it. We use the **Vite** plugin (`vite.plugins`), not PostCSS. (The earlier rolldown-vite incompatibility, `Missing field tsconfigPaths`, withastro/astro#16542, is resolved.)  |
...
```
/                         Home: overview matrix (SeasonMatrix) + state picker
/state/[code]             Per-state: list (traffic-light) + calendar (timeline), category & huntable filters
/species/[slug]           Cross-state view of one species: all 16 states, ranked open-first, with the GermanyMap
```

Home (`/`) is the **overview matrix** (`SeasonMatrix` cross-state table) plus the state picker. The card
grid is a **one-step chooser**: pick your Bundesland (on the map or the card grid) → see your state's
open species. State pages live at stable `/state/<code>` URLs, so visitors can bookmark their own state
and skip the chooser entirely on return.

`/species/[slug]` is the inverse axis: pick a species, see where it is open _right now_ across all 16
Bundesländer, with the **GermanyMap** as a fixed-species choropleth. `slug` is the taxonomy `speciesKey`
(already URL-safe). Reachable by linking the species name from the per-state list and the overview
matrix; one generic route over the taxonomy, no per-species code.

## 7. Planned components (React islands unless noted)

| Component           | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GermanyMap`        | **Fixed-species choropleth** on the species pages (props `{ matrix, speciesKey }`, fed `speciesMatrix(key)`). Shows per-state traffic-light for one species; class toggle only. No count mode or picker. Click a state → its page.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `SeasonMatrix`      | The **home page**'s lead component: cross-state grid showing species/class rows × state columns, cells coloured by `monthStatus` for a selected month (default current, `?month=`). Gray cell = species absent from that state's Jagdrecht (rare until per-state presence is modelled, §9). Species labels link to `/species/<key>`. **Responsive:** desktop-only dense table (`md`+); below `md`, one card per species/class row showing **only** the states open/with-restrictions that month as chips (status dot + code, chip → state page), the rest summarised as "+ N weitere geschlossen" (or "diesen Monat nirgends offen") — a 16-column grid is a wide-screen artefact, and the compact "where can I hunt this now" framing fits slow rural mobile, the primary audience (§2). |
...
`mergeSeasons(federal, state, taxonomy)`, and `speciesLabel`/`classLabel` helpers; `data.ts` —
`speciesMatrix(key)` (per-species cross-state matrix feed for the map). Still to add:
...
- **Overview matrix — `SeasonMatrix.tsx` + `buildMatrix` + `monthStatus`:** species/class
  rows × state columns, coloured by month (selector, `?month=`); sticky header/first column. Now the
  **home page** lead component (was `/overview`).
- **Species pages — `/species/[slug]` (`SpeciesStateList.tsx` + `getSpeciesDetail`):** the inverse axis,
  ...
  islands.
- **GermanyMap moved to species pages (#72):** the map is now a **fixed-species choropleth** mounted on
  `/species/[slug]` (props `{ matrix, speciesKey }`, fed by `speciesMatrix(key)` from `data.ts`); class
  toggle only; `/overview` removed; map count-mode/picker dropped. **105 static pages** now build
  (16 states + home + impressum + datenschutz + 86 species).
